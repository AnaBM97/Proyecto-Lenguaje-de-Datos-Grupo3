  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-shopping-cart"></span> Orden de compra </h4>' +
        '</div>' +
        ...
        '<button type="button" class="btn btn-primary ' + classCheckoutCart + '">Caja</button>' +                  
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function () {
      ...
      $cartTable.append(products.length ?
        ...
        '<td></td>' +
        '</tr>' :
        '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">El carrito está vacio</div>'
      );
    ...
    */
    ...
    $('.my-cart-checkout').on('click', targetSelector, function () {
      window.location.replace("http://localhost/ProyectoAmbienteWebC-SGrupo7/View/checkout.php");
    });

    $('#obtenerCheckout').on('click', targetSelector, function () {
      $('#obtenerCheckout').prop('disabled', true);
      var products = ProductManager.getAllProducts();
      if (!products.length) {
